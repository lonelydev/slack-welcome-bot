...
/**
 * This custom function will take the initial form input, store it in the datastore
 * and create an event trigger to listen for user_joined_channel events
 * in the specified channel.
 *
 * This function provides three properties as input_parameters.
 * These are the three pieces of information you want to pass to the datastore:
 * - the welcome message,
 * - the channel to post in,
 * - and the user ID of the person who created the message.
 */
...
export default SlackFunction(
  ...
  async ({ inputs, client }) => {
    ...
    // create a new trigger if non exists
    if (!triggers.exists) {
...
export async function findUserJoinedChannelTrigger(
  { client, channel }: { client: SlackAPIClient; channel: string },
  ...
  // return aif any matching triggers were found
